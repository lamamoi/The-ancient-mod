...
public class DeepslateforgeFeature extends Feature<NoneFeatureConfiguration> {
	public static DeepslateforgeFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new DeepslateforgeFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("the_ancient_mod:deepslateforge", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("the_ancient_mod:deepslateforge", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("the_ancient_mod", "deepslateforge"));
		if (template == null)
			return false;
		boolean anyPlaced = false;
		if ((context.random().nextInt(1000000) + 1) <= 3070) {
